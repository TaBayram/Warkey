...
public static class PoissonDiscSampling
{
    ...
    public static List<Vector2> GeneratePointsOnExisting(PoissonDiscSettings settings, float radius, List<Vector2> oldPoints) {
        float cellSize = radius / Mathf.Sqrt(2);

        System.Random random = new System.Random(settings.seed);

        int[,] grid = new int[Mathf.CeilToInt(settings.sampleRegionSize.x / cellSize), Mathf.CeilToInt(settings.sampleRegionSize.y / cellSize)];
        List<Vector2> existingPoints = new List<Vector2>(oldPoints);
        List<Vector2> points = new List<Vector2>();
        List<Vector2> spawnPoints = new List<Vector2>();
        ...
        while (spawnPoints.Count > 0) {
            ...
            for (int i = 0; i < settings.sampleCountBeforeRejection; i++) {
                ...
                if (IsValid(candidate, settings.sampleRegionSize, cellSize, radius, existingPoints, grid)) {
                    points.Add(candidate);
                    existingPoints.Add(candidate);
                    spawnPoints.Add(candidate);
                    grid[(int)(candidate.x / cellSize), (int)(candidate.y / cellSize)] = points.Count;
                    candidateAccepted = true;
                    break;
